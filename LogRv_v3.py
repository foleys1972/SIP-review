...
class CallFlowDiagram(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.message_font = QFont("Arial", 8)  # MODIFIED: Smaller font for messages
        self.timestamp_font = QFont("Arial", 6)  # NEW: Smaller font for timestamps
        self.colors = {
        ...
        self.message_items = []  # NEW: Store message items for highlighting
        self.selected_index = None  # NEW: Track selected message index
    ...
    def display_call_flow(self, call_flow, selected_index=None):  # MODIFIED: Accept selected_index
        self.scene.clear()
        self.message_items = []  # NEW: Reset message items
        self.selected_index = selected_index  # NEW: Store selected index
        
        if not call_flow:
            return
        
        # NEW: Dynamic vertical spacing based on number of messages
        message_count = len(call_flow)
        self.vertical_spacing = max(40, min(60, 1000 / (message_count + 1)))
        ...
        for i, node in enumerate(nodes):
            ...
            line = QGraphicsLineItem(x, 50, x, message_count * self.vertical_spacing + 100)
        ...
        for i, msg in enumerate(call_flow):
            ...
            pen = QPen(color, 2)
            if i == selected_index:  # NEW: Highlight selected message
                pen.setWidth(4)
            ...
            # NEW: Add message label with truncation and tooltip
            ...
            text.setPos((x1 + x2) / 2 - text_width / 2, y - 25)  # MODIFIED: Higher offset
            # NEW: Add semi-transparent background
            bg_rect = QGraphicsRectItem(text.boundingRect().adjusted(-2, -2, 2, 2))
            bg_rect.setPos(text.pos())
            bg_rect.setBrush(QBrush(QColor(255, 255, 255, 200)))
            bg_rect.setPen(QPen(Qt.NoPen))
            self.scene.addItem(bg_rect)
            self.scene.addItem(text)
            text.setToolTip(msg_label)  # NEW: Full text in tooltip
            # NEW: Store for highlighting
            self.message_items.append((arrow, arrowhead, text, bg_rect))
            
            # NEW: Add timestamp with adjusted position
            if 'timestamp' in msg:
                ...
                time_text.setPos(10, y + 5)  # MODIFIED: Below arrow
                time_bg = QGraphicsRectItem(time_text.boundingRect().adjusted(-2, -2, 2, 2))
    ...
    # NEW: Highlight a specific message
    def highlight_message(self, index):
        self.selected_index = index
        for i, (arrow, arrowhead, text, bg_rect) in enumerate(self.message_items):
            pen = arrow.pen()
            if i == index:
                pen.setWidth(4)
                pen.setStyle(Qt.DashLine)
                text.setDefaultTextColor(QColor(0, 0, 255))  # Blue for selected
                bg_rect.setBrush(QBrush(QColor(200, 200, 255, 200)))  # Light blue background
            else:
                pen.setWidth(2)
...
class SipAnalyzerApp(QMainWindow):
    ...
    def on_call_selected(self, item, column=0):
        if item.parent():
            ...
            self.call_flow_diagram.display_call_flow(messages, selected_index=msg_index)  # MODIFIED: Pass selected_index
            self.call_flow_diagram.highlight_message(msg_index)  # NEW: Highlight selected message
        else:
            call_id = item.text(0)
            if "..." in call_id:
