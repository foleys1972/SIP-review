...
from PyQt5.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, 
                             ...
                             QGraphicsPixmapItem, QButtonGroup, QRadioButton)
...
def load_config():
    # FIXED: Robust config loading for KeyError: 'themes'
    # ENHANCED: Save default config if file is missing or invalid
    default_config = {
        'highlight_codes': [],
        'recent_files': [],
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            save_config(default_config)  # Save default config to disk
...
class ParserThread(QThread):
    ...
    def parse_pcap(self, file_path):
        ...
        try:
            ...
            logging.error(f"Error reading PCAP file: {eoubt}")
    ...
    def parse_text_file(self, file_path):
        ...
        try:
            ...
            for method in SIP_METHODS:
                # FIXED: Corrected f-string syntax and invalid character
...
class CallFlowDiagram(QGraphicsView):
    ...
    def __init__(self, parent=None, main_window=None):
        ...
        self.main_window = main_window  # FIXED: Store reference to SipAnalyzerApp
        ...
        self.call_flow = None  # FIXED: Initialize call_flow storage for NameError
    
    def set_theme(self, theme_name):
        # FIXED: Fallback for missing themes (KeyError: 'themes')
        config = load_config()
        default_theme = {
            'good': [100, 200, 100],
            'info': [100, 150, 255],
    ...
    def wheelEvent(self, event):
        ...
        if self.main_window:  # FIXED: Use main_window for status_bar
    ...
    def display_call_flow(self, call_flow, selected_index=None):
        ...
        self.call_flow = call_flow  # FIXED: Store call_flow for use in highlight_message
        ...
        self.vertical_spacing = max(60, min(100, 1500 / (message_count + 1)))  # MODIFIED: Increased spacing for clarity
        ...
        node_x_positions = {}
        scene_width = 800  # MODIFIED: Default scene width for separator line
        for i, node in enumerate(nodes):
            x = i * self.horizontal_spacing + 150  # MODIFIED: Shift nodes right for seq numbers
            node_x_positions[node] = x
            label = QGraphicsTextItem(node)
            label.setPos(x - self.node_width/2, 20)  # MODIFIED: Moved down to y=20
            label.setFont(QFont("Arial", 10))
            self.scene.addItem(label)
            line = QGraphicsLineItem(x, 60, x, message_count * self.vertical_spacing + 150)  # MODIFIED: Start at y=60
            line.setPen(QPen(Qt.DashLine))
            self.scene.addItem(line)
            scene_width = max(scene_width, x + self.node_width)  # MODIFIED: Update scene width
        
        # MODIFIED: Add horizontal separator line below node labels
        separator = QGraphicsLineItem(0, 60, scene_width, 60)
        separator.setPen(QPen(Qt.DashLine))
        self.scene.addItem(separator)
        
        y = 100  # MODIFIED: Increased initial y to avoid overlap with node labels
        for i, msg in enumerate(filtered_flow):
            ...
            text.setPos((x1 + x2) / 2 - text_width / 2, y - 25)  # MODIFIED: Centered above arrow
            bg_rect = QGraphicsRectItem(text.boundingRect().adjusted(-3, -3, 3, 3))  # MODIFIED: Consistent padding
            ...
            seq_text.setPos(10, y - 25)  # MODIFIED: Fixed position on left side
            seq_bg = QGraphicsRectItem(seq_text.boundingRect().adjusted(-3, -3, 3, 3))  # MODIFIED: Consistent padding
            ...
            annotation_icon.setPos(text.pos().x() + text_width + 10, y - 25)  # MODIFIED: Right of message label
            annotation_icon.setVisible(bool(msg['annotation']))
            annotation_icon.setToolTip(msg['annotation'] if msg['annotation'] else "")
            ...
            if 'timestamp' in msg:
                ...
                time_text.setPos(scene_width - time_width - 10, y - 25)  # MODIFIED: Right side
                time_bg = QGraphicsRectItem(time_text.boundingRect().adjusted(-3, -3, 3, 3))  # MODIFIED: Consistent padding
    ...
    def highlight_message(self, index):
        ...
        if not self.call_flow:  # FIXED: Check if call_flow exists
            return
        for i, (group, arrow, arrowhead, text, bg_rect, annotation_icon) in enumerate(self.message_items):
            pen = arrow.pen()
            if index is None or self.call_flow.index(group.message) != index:  # FIXED: Use self.call_flow
                pen.setWidth(2)
...
class SipAnalyzerApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.showMaximized()  # MODIFIED: Maximize window on opening
        ...
        # Left panel: Call tree and message details
        left_widget = QWidget()
        ...
        # Right panel: Call flow diagram and controls
        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        
        # FIXED: Create call_flow_diagram first (AttributeError fix)
        self.call_flow_diagram = CallFlowDiagram(main_window=self)  # FIXED: Pass self as main_window
    ...
    def on_call_selected(self, item, column=0):
        if item.parent():
            ...
            if 0 <= msg_index < len(messages):  # FIXED: Corrected syntax error (if0 to if 0)
    ...
    def show_about(self):
        QMessageBox.about(
            ...
            "SIP Log Analyzer\nVersion 4.0\n\nA tool for analyzing SIP logs and visualizing call flows.\n\nDeveloped with PyQt5 and Scapy.\n\n© 2025"
        )
