...
from PyQt5.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, 
                              ...
                              QMenu, QAction, QMessageBox, QGraphicsPolygonItem)  # Added QGraphicsPolygonItem here
from PyQt5.QtCore import Qt, QRectF, QThread, pyqtSignal, QSize, QPointF  # Added QPointF here
from PyQt5.QtGui import QColor, QPen, QBrush, QFont, QIcon, QPolygonF  # Added QPolygonF here
from scapy.all import rdpcap, IP, TCP, UDP
from PyQt5.QtGui import QPainter
from PyQt5.QtCore import QPointF  # Add this line to import QPointF
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('sip_log_parser.log', mode='w')  # Log to file
    ]
)

CONFIG_FILE = 'highlight_config.json'
MAX_MSG_DISPLAY = 1000  # Limit max characters to prevent UI lockup
current_call_flows = {}

# Common SIP ports for more efficient packet filtering
SIP_PORTS = [5060, 5061, 5062, 5063]

# Expanded SIP method types for better identification
SIP_METHODS = [
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            return {'highlight_codes': [], 'recent_files': [], 'sip_ports': SIP_PORTS}
    return {'highlight_codes': [], 'recent_files': [], 'sip_ports': SIP_PORTS}
...
def classify_sip_message(code):
    """ Classify the SIP message by status code for visual representation """
    try:
        code_int = int(code)
        ...
        # If code is not a number (e.g., a SIP method)
        return 'method', "SIP method"

def explain_sip_message_simple(code):
    """ Return simple explanation based on SIP status code """
    return sip_status_explanations.get(code, "Unknown status")

def extract_sip_headers(payload_str):
    """Extract key SIP headers from the payload string"""
    headers = {}
    
    # Common SIP headers to extract
    header_patterns = {
        'call_id': r'Call-ID:\s*([^\r\n]+)',
        'from': r'From:\s*([^\r\n]+)',
        'to': r'To:\s*([^\r\n]+)',
...
def is_sip_packet(packet):
    """Check if a packet is likely to be a SIP packet based on port and content"""
...
def extract_sip_info(payload_str):
    """Extract SIP method or response code from payload"""
    # Check for SIP methods first
...
class ParserThread(QThread):
    """Thread for parsing files to prevent UI freezing"""
    ...
    def parse_pcap(self, file_path):
        """ Parse the PCAP file using Scapy and extract SIP messages """
        ...
        for i, packet in enumerate(packets):
            if i % 100 == 0:  # Update progress every 100 packets
                self.progress_signal.emit(int(i / total_packets * 100))
                
            if is_sip_packet(packet):
                try:
    ...
    def parse_text_file(self, file_path):
        """ Parse a text file containing SIP messages """
        ...
        try:
            ...
            # New regex pattern for better SIP message identification
            # This pattern looks for both SIP requests and responses
            sip_messages = []
            
            # Find SIP responses (status codes)
...
class CallFlowDiagram(QGraphicsView):
    """Custom widget for displaying SIP call flow diagrams"""
    ...
    def display_call_flow(self, call_flow):
        """Display a SIP call flow diagram"""
        self.scene.clear()
        ...
        # If no IPs found (text file), create nodes based on From/To headers
        if not ips:
            nodes = ["User Agent 1", "Proxy", "User Agent 2"]
        else:
            nodes = sorted(list(ips))
        
        # Draw the vertical lines for each node
        node_x_positions = {}
        ...
        for i, msg in enumerate(call_flow):
            ...
            # If src/dst not found, use placeholders
        ...
        # Adjust scene rect to show all content
        self.scene.setSceneRect(self.scene.itemsBoundingRect())
        
class MessageDetailsWidget(QTextEdit):
    """Widget to display SIP message details"""
    ...
    def display_message(self, message):
        """Display a SIP message with syntax highlighting"""
        if not message:
            self.clear()
            return
        ...
        for line in lines:
            if ': ' in line:  # This is likely a header
                parts = line.split(': ', 1)
                html += f"<span style='color:blue;font-weight:bold;'>{parts[0]}</span>: {parts[1]}<br>"
            elif line.startswith('SIP') and len(line) > 10:  # SIP response line
                html += f"<span style='color:green;font-weight:bold;'>{line}</span><br>"
            elif any(line.startswith(method) for method in SIP_METHODS):  # SIP method line
...
class SipAnalyzerApp(QMainWindow):
    ...
    def initUI(self):
        ...
        # Left side (call list and message view)
        left_widget = QWidget()
    ...
    def load_file(self, file_path):
        ...
        self.parser_thread = ParserThread(file_path, file_type)
    ...
    def display_call_flows(self, call_flows):
        """Display parsed call flows in the UI"""
        ...
        for call_id, messages in call_flows.items():
            ...
            item = QTreeWidgetItem([
                ...
                str(len(messages)),
                ""  # Will be colored instead of text
            ])
            
            # Set status color
            status_color = QColor(100, 200, 100)  # Default green
            if overall_status == "bad":
                status_color = QColor(255, 100, 100)  # Red
            elif overall_status == "warning":
                status_color = QColor(255, 200, 0)  # Yellow/amber
    ...
    def on_call_selected(self, item, column=0):
        """Handle selection of a call or message in the tree"""
        # Check if it's a message (child item)
        if item.parent():
            call_item = item.parent()
            call_id = call_item.text(0)
            if "..." in call_id:  # Get the full call ID
            ...
            # Display call flow with selected message
            self.call_flow_diagram.display_call_flow(messages)
            
        else:  # It's a call (top level item)
            call_id = item.text(0)
            if "..." in call_id:  # Get the full call ID
            ...
            # Clear message details if no message selected
            self.message_details.clear()
    
    def export_report(self):
        """Export call flows to a PDF report"""
        ...
        try:
            ...
            for call_id, messages in self.current_call_flows.items():
                ...
                for i, msg in enumerate(messages):
                    # Handle line wrapping for long fields
                ...
                # Add page break between calls if needed
                if pdf.get_y() > 250:
                    pdf.add_page()
    ...
    def export_csv(self):
        """Export call flow data to CSV format"""
        if not self.current_call_flows:
            QMessageBox.warning(self, "No Data", "There is no call flow data to export.")
            return
    ...
    def configure_ports(self):
        """Configure SIP ports for packet filtering"""
        # Declare SIP_PORTS as global before using it
        global SIP_PORTS
        ...
        # Get current ports
        current_ports = self.config.get('sip_ports', SIP_PORTS)
    
        # Create edit fields for each port
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if not new_ports:  # Ensure we have at least one port
                new_ports = [5060]
    ...
    def configure_highlights(self):
        """Configure which SIP status codes to highlight"""
        dialog = QDialog(self)
        dialog.setWindowTitle("Configure Highlights")
        layout = QVBoxLayout(dialog)
        
        # Create a grid of checkboxes for common status codes
        grid_layout = QGridLayout()
        checkboxes = {}
        
        # Get current highlight codes
...
def extract_transaction_id(msg):
    """Extract the SIP transaction ID from a message"""
    # CSeq + method/code forms a transaction
...
def identify_failed_transactions(transactions):
    """Identify failed transactions based on response codes"""
    failures = {}
    ...
    for call_id, trans in transactions.items():
        ...
        for trans_id, messages in trans.items():
            # Check if any message has error status code
            has_error = False
            final_status = None
            ...
            for msg in messages:
                ...
                if code and code.isdigit():
                    code_int = int(code)
                    if 400 <= code_int:  # 4xx, 5xx, 6xx are errors
                        has_error = True
                        final_status = code
