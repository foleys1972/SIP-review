...
class PacketAnalyzer(tk.Toplevel):
    ...
    def create_main_content(self):
        ...
        packet_y_scroll = ttk.Scrollbar(packet_frame, orient=tk.VERTICAL, command=self.packet_tree.yview)
        packet_y_scroll.pack(side=tk.RIGHT, fill=tk.Y)

        packet_x_scroll = ttk.Scrollbar(packet_frame, orient=tk.HORIZONTAL, command=self.packet_tree.xview)
        packet_x_scroll.pack(side=tk.BOTTOM, fill=tk.X)

        self.packet_tree.configure(yscrollcommand=packet_y_scroll.set, xscrollcommand=packet_x_scroll.set)
        ...
        # Split SIP Call Flow into Left (Canvas) and Right (Details)
        ...
        # SIP Call Info Panel
        self.call_info_frame = ttk.LabelFrame(self.call_flow_container, text="Call Details")
        self.call_info_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=5, pady=5)
        self.call_info_text = tk.Text(self.call_info_frame, height=20, width=40, wrap=tk.WORD)
        self.call_info_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        rtp_split_frame = ttk.Frame(self.rtp_frame)
        rtp_split_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.rtp_waveform_frame = ttk.Frame(rtp_split_frame)
        self.rtp_waveform_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # RTP Call Info Panel
        self.rtp_info_frame = ttk.LabelFrame(rtp_split_frame, text="Call Details")
        self.rtp_info_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=5, pady=5)
        self.rtp_info_text = tk.Text(self.rtp_info_frame, height=20, width=40, wrap=tk.WORD)
        self.rtp_info_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def on_rtp_stream_selected(self, event):
        ...
        if selection:
            ...
            stream = self.rtp_streams.get(stream_key)
            if stream and stream.get('associated_call_id'):
                self.update_rtp_info_panel(stream['associated_call_id'])
            else:
                self.rtp_info_text.delete(1.0, tk.END)
    ...
    def update_call_info_panel(self, call_id):
        call_data = self.sip_calls.get(call_id)
        if not call_data:
            return
        self.call_info_text.delete(1.0, tk.END)
        from_user = call_data['from']
        to_user = call_data['to']
        call_id_display = call_id
        # Try to extract dialed number from To header
        dialed_number = to_user.split('@')[0] if '@' in to_user else to_user
        line_name = from_user.split('@')[0] if '@' in from_user else from_user

        info = f"From: {from_user}\n"
        info += f"To: {to_user}\n"
        info += f"Dialed Number: {dialed_number}\n"
        info += f"Calling ID: {call_id_display}\n"
        info += f"Line Name: {line_name}\n"

        self.call_info_text.insert(tk.END, info)

    def update_rtp_info_panel(self, call_id):
        call_data = self.sip_calls.get(call_id)
        if not call_data:
            return
        self.rtp_info_text.delete(1.0, tk.END)
        ...
        self.rtp_info_text.insert(tk.END, info)
